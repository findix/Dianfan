			...
			<div class="panel panel-success">
				...
				<div class="panel-body">
					<table class="table table-hover">
						...
						<tbody>
							<%
								for (DBObject dish : QueryUtil.getOrderList()) {
							%>
						...
						: "未完成"%></td>
								<%
									if (Boolean.valueOf(dish.get("finished").toString())){
								%>
								<td><button class="btn btn-danger"
										onClick='deleteOrder("<%=dish.get("id")%>")' value="删除">删除</button></td>
								<%
									}else{
								%>
								<td><button class="btn btn-success"
										onClick='finishOrder("<%=dish.get("id")%>")' value="完成">完成</button></td>
								<%
									}
								%>
							</tr>
					...
					<script>
						function finishOrder(id) {
							window.location.href=('FinishOrder?id='+id+'&type=finish');
						}
						function deleteOrder(id) {
							window.location.href=('FinishOrder?id='+id+'&type=delete');
						}
					</script>
				</div>
			</div>
